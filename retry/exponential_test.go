...
func TestNewExponentialBackoffRetry_New(t *testing.T) {
	testCases := []struct {
		name            string
		initialInterval time.Duration
		maxInterval     time.Duration
		maxRetries      int32
		want            *ExponentialBackoffRetry
		wantErr         error
	}{
		{
			name:            "no error",
			...
			maxRetries:      5,
			want: func() *ExponentialBackoffRetry {
				s, err := NewExponentialBackoffRetry(2*time.Second, 2*time.Minute, 5)
		...
		},
		{
			...
			maxRetries:      5,
			want:            nil,
			wantErr:         fmt.Errorf("toolkit: 无效的间隔时间 %d, 预期值应大于 0", 0*time.Second),
		},
		{
			...
			maxRetries:      5,
			want:            nil,
			wantErr:         fmt.Errorf("toolkit: 无效的间隔时间 %d, 预期值应大于 0", -1*time.Second),
		},
		{
			...
			maxRetries:      5,
			want:            nil,
			wantErr:         fmt.Errorf("toolkit: 最大重试间隔的时间 [%d] 应大于等于初始重试的间隔时间 [%d] ", 1*time.Second, 5*time.Second),
		},
	}

	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			s, err := NewExponentialBackoffRetry(tt.initialInterval, tt.maxInterval, tt.maxRetries)
...
func TestExponentialBackoffRetryStrategy_Next(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			strategy: func() *ExponentialBackoffRetry {
				s, err := NewExponentialBackoffRetry(1*time.Second, 10*time.Second, 3)
				require.NoError(t, err)
				return s
			}(),
		...
		},
		{
			...
			strategy: func() *ExponentialBackoffRetry {
				s, err := NewExponentialBackoffRetry(1*time.Second, 4*time.Second, 5)
				require.NoError(t, err)
				return s
			}(),
...
// 指数退避重试策略子测试函数，无限重试
func TestExponentialBackoffRetry_Next4InfiniteRetry(t *testing.T) {
...
func ExampleExponentialBackoffRetry_Next() {
	// 注意，因为在例子里面我们设置初始的重试间隔是 1s，最大重试间隔是 5s
	// 所以在前面四次，重试间隔都是在增长的，每次变为原来的2倍。
	// 在触及到了最大重试间隔之后，就一直以最大重试间隔来重试。
	retry, err := NewExponentialBackoffRetry(time.Second, time.Second*5, 10)
	if err != nil {
		fmt.Println(err)
		return
	}
