...
func TestFixedIntervalRetryStrategy_New(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			got, err := NewFixedIntervalRetry(tt.interval, tt.maxRetries)
...
func testNext4InfiniteRetry(t *testing.T, maxRetries int32) {
	...
	s, err := NewExponentialBackoffRetry(1*time.Second, 4*time.Second, maxRetries)
...
func ExampleFixedIntervalRetry_Next() {
	retry, err := NewFixedIntervalRetry(time.Second, 3)
	if err != nil {
		fmt.Println(err)
		return
	}
